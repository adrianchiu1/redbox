...
| `debt_reference_series.csv` | 126,888 | 11 | reference series behind the register: RPI (Jan 1987=100, 1947-), French CPI ex-tobacco (base 2025, 1990-), euro-area/FR/DE HICP ex-tobacco, SONIA, Bank Rate, euro money-market and national 3m and long rates, and the BoE nominal/real/inflation/OIS month-end curves by maturity — one row per series-date |
...
| `debt_interest_reconciliation.csv` | 1,706 | 13 | interest chain: Σ register by class → CG interest (ministry) → S.1311 D.41 → GF01_7, with every bridge item and the residual per step, per country-year (V32: official_total = carried + Σ items + residual) |
| `debt_financing_reconciliation.csv` | 3,152 | 13 | financing chain: Σ net issuance by class → CG net cash requirement / Nettokreditaufnahme → S.1311 B.9 → NLB, same shape |
