...
def _official_totals(iso3: str) -> dict[str, pd.Series]:
    ...
    from the country's primary `envelope_forecast` source
    (forecast.envelopes). FRA/DEU: AMECO URTG/UUTG/UVGD. GBR: the §15 Q12
    OBR-primary default, exercisable since the OQ-6 partial unblock
    (D-S7-002) — OBR PS current receipts / TME with the OBR nominal GDP
    path, FY converted per §7.10; public sector perimeter (stable
    ~0.95-0.97 ratio to the GG totals), so the level wedge largely cancels
    in the §8.3 changes-from-base and the remainder is visible in
    resid_disagreement, never absorbed."""
    from ggfiscal.forecast.envelopes import official_envelope

    return official_envelope(iso3)
